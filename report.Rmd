...
We also display the R squared values for each test.
...
In summary, we note that the variables that affect the difference in post and pre-test scores from these least squares regression models are mainly site ( which reflects the demographic/background of teh child) and `viewcat` (frequency of viewing Sesame Street). It makes sense that kids who watched more Sesame Street, especially those that watched more than 1-2 times a week, had larger differences between their pre and post-test scores. We also see that in general, the difference between pre and post scores are higher especially for body parts, relational terms, and forms as their intercept values were the highest.
