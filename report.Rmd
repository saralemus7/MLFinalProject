...
In addition to Support Vector Machines, we also implemented Random Forest model and a logistic regression model to compare which model has the best prediction performance. In the Random Forest model, since we typically build a random forest of classification tree using $\sqrt{p}$ features from the $p = 11$ features, we set the argument $mtry = 4$. In Logistic Regression, we first built a full model that includes the 9 features and all of their pairwise interactions. Then we performed backward selection using 
AIC as the metric.
...
table.svm.cm <- kable(svmTab, caption = "Linear Kernel SVM (Class reweighted) Confusion Matrix", booktabs=T)
